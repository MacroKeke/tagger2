...
@client.on(events.NewMessage(pattern="^/cakal$"))
async def help(event):
  helptext = "**Çok özellikleri Etiket Botu Bulmaya Çalışan Grub Sahibleri @MajesteTaggerBot Size Göre:\n\n📌 5-li etiket\n📌 Emoji etiket\n📌 Tekli Etiket\n📌 Yalnız Yöneticileri etiketleme\n📌\n\n Böyle Çok özellikli @MajesteTaggerBot 'u grubunuza yönetici olarak ekleyip rahatlıkla üyelir , etiket ata bilirsiz **"
  await event.reply(helptext,
                    buttons=(
                      [Button.url('Botu Gruba Ekle➕', 'https://t.me/MajesteTaggerBot?startgroup=a')],
                    ),
                    link_preview=False
                   )
...
emoji = "Kalbim ömrüm gecem gündüzüm hayalim hayatım özledim birtanem yaşamaSebebim hayalim rüyam dünyam bebeim SeniYerim gülüm, papatyam bebem ömrüm hayatım aşkBOcegim ".split(" ")
...
emoji = "Vampir Polis Dedektif Dansöz vatandaş iskelet satıcı nişancı ölü don bombacı bukalemunun sevgili killer Joker Batman Komiser şarkıci dolandırıcı General sapık ninja Hacker kaptan dinozor".split(" ")


@client.on(events.NewMessage(pattern="^/mafiatag ?(.*)"))
async def mentionall(event):
  global anlik_calisan
  if event.is_private:
    return await event.respond("**Bu komutu gruplar ve kanallar için geçerli❗**")
...
print(">> Bot çalıyor merak etme 🚀 @MajesteSahip bilgi alabilirsin <<")
